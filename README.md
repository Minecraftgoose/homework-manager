...
- **LocalStorage API**：用于存储全局设置、主题偏好等轻量配置数据和本地版所有数据

### 设计原则

- **分端场景适配**：项目分为教师端、课堂端和本地三个独立 HTML 文件，分别针对多班级办公管理与单班级课堂展示场景进行功能裁剪，避免功能冗余，提升使用专注度
- **单文件轻量化部署**：单 HTML 文件架构，无需构建工具，直接部署至任意静态服务器或本地打开即可运行，极大降低分发与维护成本
...
2. 双击文件，使用任意现代浏览器打开
3. 如为云端班级，配置 Supabase 项目并绑定班级即可开始使用

注意：应用设置（如主题偏好）和本地版所有数据存储于浏览器本地，清除浏览器数据或更换浏览器会导致数据丢失。建议定期备份重要数据。
...
| 实时同步模块 | Supabase Realtime 订阅建立、消息处理、状态同步 | `setupRealtime()`, `unsubscribeRealtime()` 等 |
...
- **冲突处理**：编辑时实时监测云端数据变化，如出现编辑冲突立即提醒用户
...
2. **云端班级架构**：云端班级采用 Supabase 存储，以绑定码为班级唯一标识，实现数据的云端持久化与多端实时同步
...
1. **离线编辑限制**：云端班级在离线状态下无法编辑作业，本地缓存能力较弱
2. **历史版本缺失**：没有作业数据的历史版本记录，误操作后无法回退到之前的状态
3. **移动端体验**：虽然有响应式设计，但在小屏设备上的编辑体验仍有优化空间
4. **无用户体系**：基于绑定码的模式虽然便捷，但无法进行权限管理和操作审计
